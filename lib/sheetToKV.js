...
const kvToLocalization_1 = require("./kvToLocalization");
const cli = require('cli-color');
const PLUGIN_NAME = 'gulp-dotax:sheetToKV';
function sheetToKV(options) {
    const { customPinyins = {}, sheetsIgnore = /^\s*$/, verbose = false, forceEmptyToken = `__empty__`, autoSimpleKV = true, kvFileExt = '.txt', chineseToPinyin = true, indent = '    ', aliasList = {}, addonCSVPath = null, addonCSVDefaultLang = `SChinese`, } = options;
    (0, pinyin_pro_1.customPinyin)(customPinyins);
    ...
    function convert_chinese_to_pinyin(da) {
        ...
        if (match != null) {
            match.forEach((m) => {
                s = s
                    .replace(m, (0, pinyin_pro_1.pinyin)(m, { toneType: 'none', type: 'array' }).join('_'))
                    .replace('ü', 'v');
            });
        }
        return s;
    ...
    function deal_with_kv_value(value) {
        ...
        if (value == undefined)
            return '';
        if (forceEmptyToken == value)
            return '';
        return value;
    }
    function convert_row_to_kv(row, key_row) {
    ...
    let genratedFiles = [];
    function convert(file, enc, next) {
        if (file.isNull())
            return next(null, file);
        if (file.isStream())
            return next(new Error(`${PLUGIN_NAME} Streaming not supported`));
        ...
        if (file.isBuffer()) {
            ...
            const workbook = node_xlsx_1.default.parse(file.contents);
...
`;
                ...
                let generaetdFileFullname = path_1.default.join(fileDirectory, kvBaseName);
                ...
                out_put.replace(/\r\n/g, '\n');
                const kv_file = new vinyl_1.default({
                    base: file.base,
                    path: file.path,
                    basename: kvBaseName,
    ...
    function endStream() {
        if (addonCSVPath != null) {
            (0, kvToLocalization_1.pushNewLinesToCSVFile)(addonCSVPath, locTokens);
        }
        this.emit('end');
    }
    return through2_1.default.obj(convert, endStream);
}
exports.sheetToKV = sheetToKV;
